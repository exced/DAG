...
let ordonnanceur_avec_heuristique res g =
	let gc = copy g in
	begin
		markBy_chemin_critique gc;
		let rec ordonnanceur_full y z result =
			match y with
			|[] -> result
			|t::q ->
				let (y_etape, z_etape, result_etape) = etape_ordonnanceur gc (sortBy_chemin_critique gc y) z res 
				in ordonnanceur_full y_etape z_etape (result@[result_etape])
		in ordonnanceur_full (sansDep gc) [] []
	end	
;;
...
let rec etape_ordonnanceur_contrainte g y z res mem =
	let rec tri_rec y ytodo z result res mact =
		...
		| t::q ->	
			...
			if ((res < 1) || (mem < (mact + (vertex_mem t)))) then
				(ytodo@y, z, result)
			else
				(* enough *)
				let zp = t::z in
					let yp = fold_succ (fun vt vq -> if (listInclude (pred g vt) zp) then vq@[vt] else vq) g t ytodo in	
 					(tri_rec q yp zp (result@[t]) (res - 1) (mact + (vertex_mem t)))	
...
let ordonnanceur_contrainte_memoire res mem g =
	...
	begin
		...
		let rec ordonnanceur_full y z result =
			...
			|[] -> result
			|t::q ->
				let (y_etape, z_etape, result_etape) = etape_ordonnanceur_contrainte gc y z res mem
				in ordonnanceur_full (sortBy_chemin_critique gc y_etape) z_etape (result@[result_etape])
		in ordonnanceur_full (sansDep gc) [] []	
	end
;;
...
let tri_length_succ g l =
	List.sort (fun a b -> if (List.length (succ g a)) > (List.length (succ g b)) then -1 else (if (List.length (succ g a)) < (List.length (succ g b)) then 1 else 0)) l
;;

let sortBy_chemin_critique_mem g l =
	List.sort (fun a b -> if ((Mark.get a) > (Mark.get b)) then -1 else (if ((Mark.get a) < (Mark.get b)) then 1 else 0)) (tri_length_succ g l)
;;
...
let ordonnanceur_contrainte_memoire_bonus res mem g =
	...
	begin
		...
		let rec ordonnanceur_full y z result =
			...
			|[] -> result
			|t::q ->
				let (y_etape, z_etape, result_etape) = etape_ordonnanceur_contrainte_bonus gc y z res mem
				in ordonnanceur_full (sortBy_chemin_critique gc y_etape) z_etape (result@[result_etape])
		in ordonnanceur_full (sansDep gc) [] []	
	end
;;
